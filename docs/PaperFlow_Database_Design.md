...
- Mobile customer app device registration and push notification delivery
...
|---|---|---|
| Relational data (all tables above) | Microsoft SQL Server | Backed up every 24 hours (NFR-AVAIL-001) |
...
| `page_tokens` | Microsoft SQL Server | Purged by scheduled cleanup job once `expires_at` has passed (recommended: hourly job, or `ON DELETE` via TTL-style batch delete) |
| `audit_log` | Microsoft SQL Server | Retained indefinitely; no delete path exposed at the application layer |
...
CREATE TABLE users (
    ...
    role VARCHAR(50) NOT NULL CHECK (role IN ('CUSTOMER','PUBLISHER','PLATFORM_ADMIN','FINANCE_ADMIN','SYSTEM_ADMIN')),
    status VARCHAR(50) NOT NULL DEFAULT 'UNVERIFIED' CHECK (status IN ('UNVERIFIED','ACTIVE','SUSPENDED')),
...
CREATE INDEX idx_pub_status ON publishers(status);
...
CREATE TABLE editions (
    ...
    status VARCHAR(50) NOT NULL DEFAULT 'PROCESSING' CHECK (status IN ('PROCESSING','ACTIVE','ARCHIVED','SUSPENDED','FAILED')),
...
CREATE INDEX idx_edition_publisher ON editions(publisher_id);
CREATE INDEX idx_edition_status ON editions(status);
CREATE INDEX idx_edition_lang_date ON editions(language, publication_date);
...
CREATE INDEX idx_bundle_publisher ON bundles(publisher_id);
...
CREATE TABLE orders (
    ...
    order_type VARCHAR(50) NOT NULL CHECK (order_type IN ('SINGLE','BUNDLE','ARCHIVE')),
    gross_amount DECIMAL(10,2) NOT NULL CHECK (gross_amount >= 0),
    discount_amount DECIMAL(10,2) NOT NULL DEFAULT 0.00,
    paypal_order_id VARCHAR(100) NULL UNIQUE,
    status VARCHAR(50) NOT NULL DEFAULT 'PENDING' CHECK (status IN ('PENDING','CAPTURE_PENDING','COMPLETED','CANCELLED','FAILED','REFUNDED')),
    created_at DATETIME2 NOT NULL DEFAULT GETDATE(),
    updated_at DATETIME2 NOT NULL DEFAULT GETDATE(),
...
CREATE TABLE order_items (
    ...
    item_type VARCHAR(50) NOT NULL CHECK (item_type IN ('SINGLE','BUNDLE','ARCHIVE')),
...
CREATE TABLE transactions (
    ...
    confirmed_via VARCHAR(50) NOT NULL CHECK (confirmed_via IN ('CLIENT_CAPTURE','WEBHOOK')),
    created_at DATETIME2 NOT NULL DEFAULT GETDATE(),
    CONSTRAINT fk_txn_order FOREIGN KEY (order_id) REFERENCES orders(id)
);
CREATE INDEX idx_txn_paypal_ref ON transactions(paypal_reference);
...
CREATE TABLE user_edition_access (
    ...
    edition_id BIGINT NOT NULL,
    order_id BIGINT NOT NULL,
    access_source VARCHAR(50) NOT NULL CHECK (access_source IN ('SINGLE','BUNDLE','ARCHIVE')),
...
CREATE INDEX idx_access_edition ON user_edition_access(edition_id);
...
CREATE INDEX idx_token_expiry ON page_tokens(expires_at);
CREATE INDEX idx_token_lookup ON page_tokens(user_id, edition_id, page_number);

CREATE TABLE commission_config (
    id BIGINT IDENTITY(1,1) PRIMARY KEY,
    transaction_type VARCHAR(50) NOT NULL CHECK (transaction_type IN ('SINGLE','BUNDLE','ARCHIVE')),
...
CREATE INDEX idx_commission_type_date ON commission_config(transaction_type, effective_from);
